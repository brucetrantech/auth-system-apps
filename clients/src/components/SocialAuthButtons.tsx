...
export default function SocialAuthButtons({ action }: SocialAuthButtonsProps) {
	...
	const handleAppleAuth = () => {
		if (linkingInfo?.existingProviders.includes('apple.com')) {
			return handleLinkAfterSignIn(signInWithApple, 'apple');
		}
		return handleFirebaseAuth('apple', signInWithApple);
	};
	...
	const isLinkingWithApple = linkingInfo?.existingProviders.includes('apple.com');
	...
	return (
		<div className="social-buttons">
			...
			<button
				type="button"
				className={`social-btn${isLinkingWithApple ? ' social-btn-highlight' : ''}`}
			...
			>
				...
				{isLinkingWithApple ? 'Link with Apple' : `${actionText} with Apple`}
			</button>
		</div>
	);
}
